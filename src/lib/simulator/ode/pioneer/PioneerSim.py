...
import ode, xode.parser, pygame, socket, subprocess
...
##        region_calib = [0.4,-0.4]
    ...
    #Running ODE standalone with UDP communication
    #UDP port 23456
    def run_server(self):
        ...
        while 1:
            ...
            pose = self.get2DPose()
            counter = self.counter
            sock.sendto("ODE% " + str(pose), (HOST, PORT))
            received = sock.recv(1024)
            
            sname = received.split('%')
                            
            if (sname[0] == 'SServer')and (sname[1] != "ignore"):
                
                somedata = received.split('(')
                vel = somedata[1].split(',')
                vel_x = vel[0]
                vel_y_a = vel[1].split(')')
                vel_y = vel_y_a[0]
                self.setWheelSpeeds(float(vel_x.strip()), float(vel_y.strip()))
...
##    print info
##    UDPServer = subprocess.Popen(["python",os.path.join("UDPServer.py")], stderr=subprocess.PIPE, stdin=subprocess.PIPE)
...
##    sim = DiffDriveSim(standalone=1, obstaclefile=obstaclefile, regionfile="test_decomposed.regions")
    sim = DiffDriveSim(standalone=0, obstaclefile=obstaclefile,regionfile=sys.argv[1],region_calib=eval(sys.argv[2]),startingpose=eval(sys.argv[3]))
##    print sys.argv[3]    
    sim.run_server()
